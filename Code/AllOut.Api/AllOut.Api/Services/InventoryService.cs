...
namespace AllOut.Api.Services
{
    public class InventoryService : IInventoryService
    {
        ...
        public async Task<string> UpdateInventoryStatusByIDsAsync(UpdateStatusByStringIDsRequest request)
        {
            var requestID = await _request.InsertRequest(_db, request.client.UserID,
                                                              request.FunctionID,
                                                              request.RequestStatus);
        ...
        {
            var inventoriesToday = await _db.Inventories.Where(data => data.CreatedDate.Year == DateTime.Parse(Globals.EXEC_DATE).Year &&
